...
scene.background = new THREE.Color(0xd9f5ff); // Un gris bleuté très clair pour plus de modernité
...
// Si ton sol est grand, la texture va s'étirer. On la fait donc se répéter.
[baseColor, normalMap, roughnessMap].forEach((tex) => {
  tex.wrapS = THREE.RepeatWrapping;
  tex.wrapT = THREE.RepeatWrapping;
  tex.repeat.set(4, 4); // Répète la texture 4 fois sur le disque
});
...
function createNoiseTexture(size = 256) {
  ...
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext('2d'); // On récupère le contexte ici

  // On crée les données de l'image directement depuis le contexte
  const imageData = ctx.createImageData(size, size); 
  
  for (let i = 0; i < imageData.data.length; i += 4) {
    // Génère un gris aléatoire entre 180 et 255 pour un grain subtil
  ...
  // Augmente ces chiffres pour un grain plus fin (ex: 50, 50)
...
const groundGeometry = new THREE.CircleGeometry(groundRadius, 64); // 64 segments pour un bord bien lisse
const groundMaterial = new THREE.MeshStandardMaterial({ 
  color: 0xb1Ed8c, 
  roughness: 0.8,
  // On utilise le bruit comme "map" pour la couleur
  // et comme "roughnessMap" pour que le grain réagisse à la lumière
  map: noiseTex,
  roughnessMap: noiseTex,
  side: THREE.DoubleSide// Pour voir le dessous du cercle si la caméra tourne
});
...
hemiFolder.add(hemiParams, 'intensity', 0, 3, 0.01)
  .onChange(value => hemiLight.intensity = value);

hemiFolder.addColor(hemiParams, 'skyColor')
  .onChange(value => hemiLight.color.set(value));

hemiFolder.addColor(hemiParams, 'groundColor')
...
dirFolder.add(dirParams, 'intensity', 0, 5, 0.01)
  .onChange(value => dirLight.intensity = value);

dirFolder.add(dirParams, 'x', -20, 20, 0.1)
  .onChange(value => dirLight.position.x = value);

dirFolder.add(dirParams, 'y', 0, 20, 0.1)
  .onChange(value => dirLight.position.y = value);

dirFolder.add(dirParams, 'z', -20, 20, 0.1)
  .onChange(value => dirLight.position.z = value);

dirFolder.add(dirParams, 'bias', -0.01, 0.01, 0.0001)
  .onChange(value => dirLight.shadow.bias = value);
...
shadowFolder.add(shadowParams, 'mapSize', 256, 4096, 256)
  .onChange(value => {
...
shadowFolder.add(shadowParams, 'top', 0, 50)
  .onChange(v => dirLight.shadow.camera.top = v);

shadowFolder.add(shadowParams, 'bottom', -50, 0)
  .onChange(v => dirLight.shadow.camera.bottom = v);

shadowFolder.add(shadowParams, 'left', -50, 0)
  .onChange(v => dirLight.shadow.camera.left = v);

shadowFolder.add(shadowParams, 'right', 0, 50)
  .onChange(v => dirLight.shadow.camera.right = v);

shadowFolder.add(shadowParams, 'near', 0.1, 10)
  .onChange(v => dirLight.shadow.camera.near = v);

shadowFolder.add(shadowParams, 'far', 1, 100)
  .onChange(v => dirLight.shadow.camera.far = v);
...
loader.load('/models/pingoui.glb', (gltf) => {
  ...
  model.traverse((child) => {
    ...
    child.geometry.computeVertexNormals();
      // Améliore le rendu des textures
      if (child.material) child.material.depthWrite = true;
          // Optionnel : shading doux
...
// --- CONFIGURATION ET RÉPARTITION DES POMMES ---

const numberOfApples = 20; // Nombre de pommes à disperser
const applesArray = []; // Tableau pour stocker les clones (utile pour les collisions plus tard)


// Rayon de dispersion légèrement inférieur pour éviter que les pommes ne soient trop au bord
const dispersionRadius = groundRadius - 1; 

// CHARGEMENT DU MODÈLE ORIGINAL DE LA POMME
loader.load('/models/apple.glb', (gltf) => {
  const originalApple = gltf.scene;

  // 1. Configuration de la pomme "mère" (taille, ombres)
  // On ne l'ajoute pas à la scène directement, elle sert de modèle pour les clones
  originalApple.scale.set(0.4, 0.4, 0.4); // Ajuste l'échelle selon ton modèle
  ...
  // 2. Boucle pour créer et positionner les 10 clones
  for (let i = 0; i < numberOfApples; i++) {
    // A. Cloner le modèle original
    const appleClone = originalApple.clone();

    // B. Calculer une position aléatoire à l'intérieur du cercle
    // On utilise la méthode "Polar Coordinates to Cartesian" pour une répartition uniforme
    const angle = Math.random() * Math.PI * 2; // Angle aléatoire entre 0 et 360 degrés
    
    // Rayon aléatoire (Math.sqrt assure une distribution plus uniforme vers le centre)
    const radius = Math.sqrt(Math.random()) * dispersionRadius; 

    // Conversion en coordonnées X et Z pour Three.js
    const randomX = Math.cos(angle) * radius;
    const randomZ = Math.sin(angle) * radius;

    // C. Positionner le clone
    // Comme demandé : Y = -0.1 pour qu'elle soit posée sur le sol vert
    appleClone.position.set(randomX, -0.155, randomZ);

    // D. Optionnel : Ajouter une rotation aléatoire pour plus de réalisme
    // (Toutes les pommes ne tombent pas dans le même sens)
    appleClone.rotation.y = Math.random() * Math.PI * 2; // Rotation aléatoire sur elle-même

    // E. Ajouter la pomme à la scène et au tableau
    scene.add(appleClone);
    applesArray.push(appleClone); // Stocke le clone pour l'utiliser dans l'animation/physique
  }
